...
let currentView = sessionStorage.getItem('admin_view') || 'registrations';
        ...
        <tbody class="divide-y divide-neutral-200">
          ${orders.length === 0 ? '<tr><td colspan="6" class="text-center py-10 text-neutral-500">No orders found.</td></tr>' :
            ...
            <tr class="hover:bg-neutral-50" id="order-row-${order.id}">
              ...
              <td class="py-4 px-2"><button class="text-primary-600 hover:underline text-sm">Details</button></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
...
export async function renderAdmin() {
  ...
  window.handleStatusChange = async (orderId, newStatus) => {
    ...
    alert(`Order #${orderId.toString().slice(-6)} status updated to ${newStatus}`);
  };
